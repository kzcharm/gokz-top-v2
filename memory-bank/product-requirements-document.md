...
- Authenticated players can generate a short-lived QQ binding code from `/settings/binding-code` and send `/bind <code>` to the QQ bot to prove Steam-account ownership; a superuser configures the shared bot-verification secret at `/admin/settings`, the website stays stateless for this flow, the code keeps the `KZTOP` prefix while using a compact fixed-length alphanumeric body, and the bot owns the final QQ binding state.
