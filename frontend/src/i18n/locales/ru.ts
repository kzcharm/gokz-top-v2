export const ru = {
  common: {
    appName: "GOKZ.TOP",
    unknown: "Неизвестно",
    notAvailable: "N/A",
    yes: "Да",
    no: "Нет",
    none: "Нет",
    tier: "Tier",
    all: "Все",
    loading: "Загрузка",
    loadingTotal: "загрузка общего числа",
    save: "Сохранить",
    cancel: "Отмена",
    close: "Закрыть",
    delete: "Удалить",
    edit: "Изменить",
    add: "Добавить",
    search: "Поиск",
    clear: "Очистить",
    open: "Открыть",
    copy: "Скопировать",
    playRunReplay: "Открыть реплей рана",
    playJumpReplay: "Открыть реплей прыжка",
    noMatches: "Совпадений не найдено.",
    copied: "{{label}} скопировано",
    copyFailed: "Не удалось скопировать {{label}}",
    retry: "Повторить",
    refresh: "Обновите страницу и попробуйте снова.",
    success: "Готово!",
    error: "Что-то пошло не так!",
    goBack: "Назад",
    goHome: "На главную",
    placeholder: "Заполнитель",
    unknownCountry: "Неизвестная страна",
    validationError: "Проверьте форму и попробуйте снова.",
  },
  labels: {
    home: "Главная",
    records: "Рекорды",
    unfinished: "Незавершённые",
    stats: "Статистика",
    player: "Игрок",
    map: "Карта",
    replay: "Реплей",
    mode: "Режим",
    tier: "Сложность",
    tps: "TPs",
    time: "Время",
    points: "Очки",
    number: "№",
    prev: "Пред.",
    rating: "Rating",
    playtime: "Время в игре",
    server: "Сервер",
    datetime: "Дата",
    updated: "Обновлено",
    ratings: "Оценки",
    comment: "Комментарий",
    lastPlayed: "Последняя игра",
    dateJoined: "Дата регистрации",
    profileViews: "Просмотры профиля",
    likes: "Лайки",
    followers: "Подписчики",
    following: "Подписки",
    maps: "Карты",
    rank: "Место",
    overall: "Общее",
    gameplay: "Геймплей",
    visuals: "Визуал",
    stage: "Этап",
    wrTime: "WR время",
    wrGap: "Разрыв с WR",
  },
  language: {
    label: "Язык",
    select: "Выбрать язык",
    current: "Текущий язык: {{language}}.",
    options: {
      en: "English",
      "zh-CN": "简体中文",
      ru: "Русский",
    },
  },
  theme: {
    toggle:
      "Переключить на {{theme}} режим. Щёлкните правой кнопкой для дополнительных вариантов.",
    toggleShort: "Переключить тему",
    light: "Светлая",
    dark: "Тёмная",
    system: "Системная",
  },
  nav: {
    servers: "Серверы",
    profile: "Профиль",
    leaderboards: "Таблицы лидеров",
    dashboard: "Панель",
    maps: "Карты",
    live: "Live",
    media: "Видео",
    bans: "Баны",
    polls: "Опросы",
    joinDiscord: "Присоединиться к Discord",
    joinDiscordHelp: "Возникли проблемы? Присоединяйтесь к нашему Discord.",
    notifications: "Уведомления",
    settings: "Настройки",
    admin: "Админ",
    users: "Пользователи",
    players: "Игроки",
    socialLinks: "Соцсети",
    playerSessions: "Сессии игроков",
  },
  serverGlobalStatus: {
    title: "Проверка GlobalAPI",
    apiKey: "Ключ API",
    plugins: "Плагины",
    settingsEnforcer: "Проверка настроек",
    map: "Карта",
    eligibleAria: "Записи GlobalAPI включены",
    ineligibleAria: "Записи GlobalAPI недоступны или не проверены",
  },
  footer: {
    joinDiscord: "Присоединяйтесь к нам в Discord",
  },
  polls: {
    votes: "голосов",
    vote: "Проголосовать",
    changeVote: "Изменить голос",
    signInToVote: "Войдите, чтобы проголосовать",
    closed: "Закрыт",
    moreOptions: "+{{count}} вариантов",
    maxVotes: "Максимум вариантов: {{count}}",
    maxVotesUnlimited: "Максимум вариантов: без ограничений",
    voteEnds: "Голосование завершится",
    loading: "Загрузка опроса…",
    unableToLoad: "Не удалось загрузить опрос",
    unableToLoadDescription: "Возможно, этот опрос больше недоступен.",
    filters: { all: "Все опросы", active: "Активные", closed: "Закрытые" },
    sort: {
      created: "Новые",
      activity: "Последняя активность",
      votes: "Больше голосов",
    },
  },
  updates: {
    title: "Обновления",
    description: "Последние релиз-ноты из публичного репозитория GOKZ.TOP.",
    openReleaseNotes: "Открыть релиз-ноты",
    errorTitle: "Не удалось загрузить обновления",
    errorBody: "Релиз-ноты GitHub сейчас недоступны.",
    empty: "Пока нет опубликованных релизов.",
    noNotes: "Для этой версии нет релиз-нотов.",
    viewOnGitHub: "Открыть на GitHub",
    sections: {
      Features: "Новое",
      Fixes: "Исправления",
      Other: "Прочее",
    },
  },
  notifications: {
    title: "Уведомления",
    open: "Открыть уведомления",
    unread: "Не прочитано",
    unreadCount: "Не прочитано: {{count}}",
    markAllRead: "Отметить все прочитанными",
    empty: "Пока нет уведомлений.",
    someone: "Кто-то",
    unknownMap: "Неизвестная карта",
    events: {
      profileLike: "{{actor}} поставил лайк вашему профилю.",
      profileLikeAction: "поставил лайк вашему профилю.",
      profileComment: "{{actor}} прокомментировал ваш профиль.",
      profileCommentAction: "прокомментировал ваш профиль.",
      playerFollow: "{{actor}} подписался на вас.",
      playerFollowAction: "подписался на вас.",
      wrBeaten: "{{actor}} побил ваш WR на {{map}}.",
      wrBeatenAction: "побил ваш WR на {{map}}.",
      wrBeatenActionPrefix: "побил ваш WR на",
      wrBeatenActionSuffix: ".",
      wrBeatenDetail: "{{scope}} {{type}} - {{time}}",
      playerReportAction: "отправил жалобу на игрока.",
      mapReviewCommentDeletedAction:
        "ваш комментарий к отзыву о карте {{map}} был удалён.",
      mapReviewCommentDeletedActionPrefix: "ваш комментарий к отзыву о карте",
      mapReviewCommentDeletedActionSuffix: "был удалён.",
      recordContext: "рекорд",
      generic: "Новое уведомление",
    },
  },
  titles: {
    login: "Вход",
    servers: "Серверы",
    maps: "Карты",
    live: "Live",
    media: "Видео",
    bans: "Баны",
    notifications: "Уведомления",
    leaderboards: "Таблицы лидеров",
    settings: "Настройки",
    dashboard: "Панель",
    profile: "Профиль",
    profileRuns: "Забеги профиля",
    profileStats: "Статистика профиля",
    profileUnfinished: "Незавершённые карты",
    profileComments: "Комментарии профиля",
    adminMaps: "Админ карт",
    adminPlayers: "Админ игроков",
    adminUsers: "Админ пользователей",
    adminServers: "Админ серверов",
    adminPlayerSessions: "Админ сессий игроков",
    adminPlayerSocialLinks: "Админ соцсетей",
    polls: "Опросы",
    adminPolls: "Админ опросов",
    adminSettings: "Настройки администратора",
  },
  adminSettings: {
    title: "Настройки приложения",
    communityLinks: {
      title: "Ссылки сообщества",
      description:
        "Показывать ссылку Discord в панели навигации. Отключите, чтобы переместить её в нижний колонтитул.",
      showInNavbar: "Показывать ссылки сообщества в панели навигации",
      toasts: {
        updated: "Расположение ссылок сообщества обновлено.",
      },
    },
    globalapiRecordsSync: {
      title: "Синхронизация рекордов GlobalAPI",
      description:
        "Импортировать рекорды из GlobalAPI в этом развёртывании. Отключите в staging, чтобы избежать повторной синхронизации; после включения импорт продолжится при следующей проверке планировщика.",
      enabled: "Синхронизировать рекорды из GlobalAPI",
      toasts: {
        updated: "Настройка синхронизации рекордов GlobalAPI обновлена.",
      },
    },
    qqBinding: {
      title: "Секрет привязки QQ",
      description:
        "Скопируйте этот секрет в QQ-бота, который проверяет коды привязки игроков. Сайт не хранит привязки QQ.",
      secretLabel: "Секрет привязки QQ",
      secretMasked: "Секрет привязки QQ скрыт",
      status: {
        configured: "Настроено",
        unconfigured: "Не настроено",
      },
      createdAt: "Создан",
      updatedAt: "Последняя ротация",
      actions: {
        generate: "Создать секрет",
        reveal: "Показать секрет",
        rotate: "Сменить секрет",
        revoke: "Отозвать секрет",
      },
      confirm: {
        rotate: {
          title: "Сменить секрет привязки QQ?",
          description:
            "Это сразу сделает недействительными все коды привязки, выданные текущим секретом. Обновите секрет в QQ-боте.",
          confirm: "Сменить секрет",
        },
        revoke: {
          title: "Отозвать секрет привязки QQ?",
          description:
            "Это сразу отключит создание кодов привязки QQ и сделает недействительными все активные коды.",
          confirm: "Отозвать секрет",
        },
      },
      toasts: {
        generated: "Секрет привязки QQ создан.",
        rotated: "Секрет привязки QQ сменён.",
        revoked: "Секрет привязки QQ отозван.",
      },
    },
  },
  errors: {
    notFoundTitle: "Упс!",
    notFoundMessage: "Страница, которую вы ищете, не найдена.",
    errorTitle: "Ошибка",
    errorMessage: "Что-то пошло не так. Попробуйте ещё раз.",
    loadFailed: "Не удалось загрузить: {{target}}",
    loadLiveFailed: "Не удалось загрузить список трансляций",
    loadLiveBody: "Страница Live не загрузилась из API.",
    mapLeaderboardFailed: "Не удалось загрузить таблицу лидеров карты",
    mapReviewsFailed: "Не удалось загрузить отзывы о карте",
    mapsFailed: "Не удалось загрузить карты",
    bansFailed: "Не удалось загрузить баны",
    recordDistributionsFailed: "Не удалось загрузить распределение рекордов",
    pinnedRecordRanksFailed: "Не удалось загрузить ранги закреплённых рекордов",
    completionProgressFailed: "Не удалось загрузить прогресс прохождения",
    clipboardUnavailable: "Буфер обмена недоступен.",
  },
  auth: {
    login: "Войти",
    logout: "Выйти",
    userAvatarAlt: "Аватар {{name}}",
    anonymousUser: "Пользователь",
  },
  analyticsConsent: {
    message:
      "Мы используем Google Analytics, чтобы улучшать ваш опыт. Вы принимаете использование cookies?",
    accept: "Принять",
    reject: "Отклонить",
  },
  dashboard: {
    records: "Рекорды",
    reviews: "Отзывы",
    wrs: {
      tab: "WR",
      firstWr: "Первый WR",
      previousDelta: "-{{delta}} от прошлого",
      previousPlayerPrefix: "-{{delta}} (пред.",
      previousPlayerSuffix: ")",
      newAvailable: "Доступны новые WR",
      preparing: "История последних WR подготавливается. Загляните позже.",
      loadFailed: "Не удалось загрузить последние WR. Обновите страницу.",
      noMatches: "Нет последних WR, соответствующих фильтрам.",
      totalLabel: "WR",
      filters: {
        map: "Фильтр WR по карте",
        tier: "Тир",
        type: "Переключить NUB/PRO WR",
        noMaps: "Карты не найдены.",
      },
    },
  },
  settings: {
    title: "Настройки",
    tabs: {
      profile: "Мой профиль",
      bindingCode: "Код привязки",
      notifications: "Уведомления",
      socialLinks: "Соцсети",
      webhooks: "Webhooks",
      appearance: "Оформление",
    },
    profile: {
      title: "Профиль",
      aliasInfo:
        "Задайте псевдоним, чтобы имя в таблице лидеров оставалось одинаковым, даже если имя Steam изменится. Так другим будет легче вас узнавать.",
      aliasInfoAria: "Информация о псевдониме",
      available: "Доступно",
      soon: "скоро",
      enabled: "Включено",
      disabled: "Отключено",
      clearCountry: "Очистить страну",
      actions: {
        edit: "Редактировать",
        save: "Сохранить",
        cancel: "Отмена",
      },
      fields: {
        steamName: "Имя Steam",
        steamId64: "SteamID64",
        alias: "Псевдоним",
        customId: "Custom ID",
        countryRegion: "Местоположение",
        primaryScope: "Основной scope",
        wrBasedProCompletion: "PRO-прогресс по WR",
        favoriteServer: "Любимый сервер",
      },
      placeholders: {
        alias: "Псевдоним",
        customId: "custom-id",
        country: "Выберите страну",
        primaryScope: "Выберите основной scope",
        favoriteServer: "Выберите любимый сервер",
      },
      favoriteServer: {
        none: "None",
      },
      fallbacks: {
        notSet: "Не задано",
      },
      errors: {
        aliasBlank: "Псевдоним не может быть пустым.",
        aliasInvalidChars:
          "Псевдоним может содержать только английские буквы, цифры, пробелы, '-' и '_'.",
        customIdBlank: "Custom ID не может быть пустым.",
        customIdInvalidChars:
          "Custom ID может содержать только английские буквы, цифры, '-' и '_'.",
        customIdMissingLetter:
          "Custom ID должен содержать хотя бы одну английскую букву.",
        countryCleared: "Страну нельзя очистить.",
      },
      toast: {
        updated: "Настройки профиля обновлены.",
      },
      qqBinding: {
        title: "Код привязки QQ",
        codeLabel: "Код привязки",
        tooltip:
          "Если вы не из Китая, эта функция вам, скорее всего, не понадобится.",
        tooltipAriaLabel: "О кодах привязки QQ",
        expiresAt: "Срок действия",
        actions: {
          generate: "Сгенерировать код привязки QQ",
        },
        toast: {
          generated: "Код привязки QQ создан.",
        },
      },
    },
    appearance: {
      themeTitle: "Тема",
      themeDescription:
        "Выберите, как приложение должно выглядеть в этом браузере.",
      defaultPage: "Страница по умолчанию",
      selectDefaultPage: "Выберите страницу по умолчанию",
      colorTheme: "Цветовая тема",
      selectTheme: "Выберите тему",
      activeTheme: "Активная тема",
      dateTimeTitle: "Дата и время",
      dateTimeDescription:
        "Настройте формат времени по умолчанию для этого браузера. Секунды и относительное время по-прежнему определяются конкретным экраном.",
      formatter: "Формат даты и времени",
      selectDateTimeFormat: "Выберите формат даты и времени",
      timeFormat: "Формат времени",
      selectTimeFormat: "Выберите формат времени",
      recordTimestamp: "Время рекорда",
      updateTimestamp: "Время обновления",
      relativeTimestamp: "Относительное время",
      playerDisplayTitle: "Отображение игрока",
      playerDisplayDescription:
        "Настройте, как компактные компоненты игрока выглядят в этом браузере.",
      countryFlag: "Флаг страны",
      countryFlagDescription: "Показывать страну игрока рядом с именем.",
      ratingIcon: "Значок рейтинга",
      ratingIconDescription:
        "Показывать значок уровня рейтинга рядом с игроком.",
      ratingIconScope: "Источник значка рейтинга",
      selectRatingIconScope: "Выберите источник значка рейтинга",
      playerDisplayPreview: "Предпросмотр",
      playerDisplayPreviewUnavailable: "Предпросмотр недоступен.",
      ratingIconScopes: {
        primary: "Основной скоуп",
        global: "Скоуп навбара",
      },
      defaultPages: {
        servers: "Серверы",
        profile: "Мой профиль",
        leaderboards: "Таблицы лидеров",
        maps: "Карты",
        live: "Live",
      },
      ratingIconScopeDescriptions: {
        primary: "Использовать рейтинг основного скоупа каждого игрока.",
        global: "Использовать скоуп, выбранный в навбаре.",
      },
      themeDescriptions: {
        light: "Всегда использовать светлую тему.",
        dark: "Всегда использовать тёмную тему.",
        system: "Автоматически следовать теме устройства.",
      },
    },
  },
  profile: {
    tabs: {
      home: "Главная",
      runs: "Забеги",
      unfinished: "Незавершённые",
      stats: "Статистика",
      jumpstats: "Jumpstats",
      comments: "Комментарии",
      friends: "Друзья",
      maps: "Карты",
    },
    searchAria: "Поиск игроков",
    searchPlaceholder: "Искать игрока ...",
    clearSearch: "Очистить поиск игрока",
    searchingPlayers: "Поиск игроков...",
    searchUnavailable: "Сейчас поиск игроков недоступен.",
    noPlayers: "Игроки не найдены.",
    zoomAvatar: "Увеличить аватар {{name}}",
    avatarAlt: "Аватар {{name}}",
    avatarAltEnlarged: "Увеличенный аватар {{name}}",
    openSteamProfile: "Открыть профиль Steam игрока {{name}}",
    socialUnverified: "не подтверждено",
    unknownDate: "Неизвестная дата",
    unavailable: "Недоступно",
    unranked: "Вне рейтинга",
    regionFallback: "Регион",
    summary: {
      steamId64: "SteamID64",
      dateJoined: "Дата регистрации",
      lastPlayed: "Последняя игра",
      playtime: "Время в игре",
      ljPb: "LJ PB",
      favoriteServer: "Любимый сервер",
      profileViews: "Просмотры профиля",
      likes: "Лайки",
      followers: "Подписчики",
      following: "Подписки",
      global: "Глобально",
      regional: "Регион",
      rating: "Rating",
    },
    ratingLadder: {
      title: "Ранги рейтинга",
      description: "Текущий рейтинг: {{rating}}",
      ratingLabel: "Рейтинг",
    },
    records: {
      showWrTime: "Показывать время WR",
      showWrGap: "Показывать разрыв с WR",
      ratingContributionInfo: "Информация о вкладе в рейтинг",
      ratingContributionTooltip:
        "Сырой рейтинг, который эта запись добавляет к рейтингу игрока.",
    },
    adminRecovery: {
      title: "Восстановление администратора",
      description:
        "После разбана сначала пересчитайте оценочные очки PB, затем рейтинг игрока.",
      recalculatePoints: "Пересчитать оценочные очки",
      recalculateRating: "Пересчитать рейтинг",
      pointsRecalculated: "Оценочные очки PB пересчитаны.",
      pointsFailed: "Не удалось пересчитать оценочные очки PB.",
      ratingRecalculated: "Рейтинг игрока пересчитан.",
      ratingFailed: "Не удалось пересчитать рейтинг игрока.",
    },
    skillRadar: {
      title: "Радар навыков",
      ariaLabel: "Радар навыков профиля",
      relativeAriaLabel: "Относительный радар навыков профиля",
      tooltip:
        "Относительный вид сохраняет глобальный радиус сильнейшего навыка и увеличивает различия внутри этого контура. Радиус слабейшего навыка также учитывает его абсолютный рейтинг, поэтому навык около 11 остаётся у внешнего края. Числа всегда остаются глобальными рейтингами и округляются вниз до двух знаков. Уровни 2–10 соответствуют всё более узким верхним процентилям, а максимальная групповая отметка остаётся ниже 11. Недоступные оценки показаны как —.",
      tooltipAria: "Как рассчитываются рейтинги навыков",
      scaleAria: "Шкала радара навыков",
      globalView: "Глобально",
      relativeView: "Относительно",
      globalHint: "Общая шкала 0–11 для сравнения игроков.",
      relativeHint:
        "Внешний размер глобальный; абсолютная сила ограничивает провал.",
      boxtech: "Boxtech",
      strafe: "Стрейф",
      bhop: "Bhop",
      climb: "Climb",
      ladder: "Лестница",
      slide: "Слайд",
    },
    socialDialog: {
      title: "Связи профиля",
      likesTab: "Лайки {{count}}",
      followersTab: "Подписчики {{count}}",
      followingTab: "Подписки {{count}}",
      emptyLikes: "Пока нет лайков.",
      emptyFollowers: "Пока нет подписчиков.",
      emptyFollowing: "Этот игрок пока ни на кого не подписан.",
      loadFailed: "Не удалось загрузить этот список.",
      loadMore: "Загрузить ещё",
      pageHint:
        "Показывается до {{count}} игроков на страницу. Нажмите на любую строку, чтобы открыть профиль.",
    },
    likes: {
      alreadyLikedToday: "Этот профиль можно лайкнуть только один раз в день.",
      likeFailed: "Не удалось поставить лайк этому игроку.",
    },
    follow: {
      follow: "Подписаться",
      unfollow: "Отписаться",
      updateFailed: "Не удалось обновить подписку.",
    },
    history: {
      menuAction: "История профиля",
      title: "История профиля",
      description:
        "Просмотрите ранее сохранённые имена и аватары профиля этого игрока.",
      empty: "Для этого игрока пока нет записанной истории профиля.",
      loadFailed: "Не удалось загрузить историю профиля.",
      missingName: "Неизвестное имя",
    },
    friends: {
      syncButton: "Синхронизировать",
      syncing: "Синхронизация...",
      synced: "Друзья синхронизированы",
      syncFailed: "Не удалось синхронизировать друзей",
      kzRatio:
        "{{kzFriends}} / {{totalFriends}} ({{percent}}% друзей играют в KZ)",
      sortFields: {
        name: "Имя",
        steamid64: "SteamID64",
        country: "Страна",
        rating: "Рейтинг",
        lastPlayed: "Последняя игра",
      },
      loadFailedTitle: "Не удалось загрузить друзей",
      loadFailedBody: "Обновите страницу и попробуйте снова.",
      empty: "Для этого игрока не найдено друзей KZ.",
      privateProfileTitle: "Профиль Steam закрыт",
      privateProfileBody:
        "Профиль Steam этого игрока не является публичным, поэтому список друзей нельзя синхронизировать.",
      privateProfileOwnerBody:
        "Ваш профиль Steam закрыт. Только вы можете видеть последний кэшированный список друзей ниже; обновить его можно будет после открытия профиля.",
      privateFriendsTitle: "Список друзей Steam скрыт",
      privateFriendsBody:
        "Список друзей Steam этого игрока скрыт, поэтому друзей нельзя синхронизировать.",
      privateFriendsOwnerBody:
        "Ваш список друзей Steam скрыт. Только вы можете видеть последний кэшированный список ниже; обновить его можно будет после открытия списка друзей.",
    },
    maps: {
      loadFailedTitle: "Не удалось загрузить карты",
      loadFailedBody: "Обновите страницу и попробуйте снова.",
      empty: "Для этого игрока не найдены созданные им карты.",
    },
    comments: {
      loginTitle: "Для комментария нужен вход",
      loginBody:
        "Войдите через Steam, чтобы оставить комментарий в этом профиле.",
      formLabel: "Оставить комментарий",
      totalLabel: "комментариев",
      placeholder: "Напишите что-то полезное, уважительное и по делу.",
      submit: "Опубликовать комментарий",
      submitting: "Публикация...",
      submitSuccess: "Комментарий опубликован",
      submitFailed: "Не удалось опубликовать комментарий",
      submitFailedTitle: "Не удалось опубликовать комментарий",
      deleteSuccess: "Комментарий удалён",
      deleteFailed: "Не удалось удалить комментарий",
      deleteFailedTitle: "Не удалось удалить комментарий",
      deleteConfirm: "Удалить этот комментарий?",
      loadFailedTitle: "Не удалось загрузить комментарии",
      loadFailedBody: "Обновите страницу и попробуйте снова.",
      empty: "Комментариев к профилю пока нет.",
    },
    jumpstats: {
      units: "units",
      loadFailedTitle: "Не удалось загрузить jumpstats",
      loadFailedBody: "Обновите страницу и попробуйте снова.",
      empty: "Для этого игрока jumpstats не найдены.",
      breakdownTitle: "Разбивка по стрейфам",
      edge: "Edge",
      deviation: "Deviation",
      mapDependentOmitted:
        "Данные, зависящие от карты, скрыты: Edge, Deviation.",
      summary: {
        strafes: "Strafes",
        sync: "Sync",
        pre: "Pre",
        max: "Max",
        width: "Width",
        height: "Height",
        airtime: "Airtime",
        offset: "Offset",
        w: "W",
        ol: "OL",
        da: "DA",
        crouched: "Crouched",
      },
      table: {
        sync: "Sync",
        gain: "Gain",
        loss: "Loss",
        airtime: "Airtime",
        width: "Width",
        ol: "OL",
        da: "DA",
      },
    },
    activity: {
      title: "Активность",
      latest: "Недавнее",
      latestRange: "последние 365 дней",
      loadFailed: "Сейчас не удаётся загрузить дневную активность.",
      emptyState: "В период {{range}} не найдено отправленных рекордов.",
      less: "Меньше",
      more: "Больше",
      cellTooltip_one: "{{count}} рекорд {{date}} UTC",
      cellTooltip_other: "{{count}} рекордов {{date}} UTC",
    },
    duration: {
      previousYear: "Перейти к предыдущему году",
      yearInput: "Выбранный год",
      nextYear: "Перейти к следующему году",
    },
    distribution: {
      loadFailedTitle: "Не удалось загрузить распределение рекордов",
      loadFailedBody: "Обновите страницу и попробуйте снова.",
      nubTitle: "Распределение очков NUB",
      proTitle: "Распределение очков PRO",
      tooltipCount_one: "{{count}} рекорд",
      tooltipCount_other: "{{count}} рекордов",
    },
    pinned: {
      loadFailedTitle: "Не удалось загрузить ранги закреплённых рекордов",
      loadFailedBody: "Обновите страницу и попробуйте снова.",
      title: "Закреплённые рекорды",
      count: "{{count}} из 6",
      rankUnavailable: "Ранг недоступен",
      empty: "Для этой области закреплённые рекорды не найдены.",
      unpin: "Открепить этот рекорд",
    },
    hiddenMaps: {
      showToggle: "Показывать скрытые рекорды",
      hideAction: "Скрыть эту карту",
      unhideAction: "Вернуть эту карту",
      hidden: "Карта скрыта из ваших забегов",
      unhidden: "Карта возвращена в ваши забеги",
      hideFailed: "Не удалось скрыть карту",
      unhideFailed: "Не удалось вернуть карту",
    },
    completion: {
      loadFailedTitle: "Не удалось загрузить прогресс прохождения",
      loadFailedBody:
        "Не удалось загрузить полосы прогресса профиля. Обновите страницу и попробуйте снова.",
      nubTitle: "Прогресс NUB",
      proTitle: "Прогресс PRO",
      averageShort: "avg",
      trophies: {
        gold: "золотой",
        silver: "серебряный",
        bronze: "бронзовый",
      },
      trophyAlt: "{{trophy}} кубок",
    },
    unfinished: {
      ariaLabel: "{{type}} незавершённые",
      mapsCount_one: "{{count}} карта",
      mapsCount_other: "{{count}} карт",
      mapSearchAria: "Поиск незавершённой карты",
      mapSearchPlaceholder: "Искать карту",
      filterTierAria: "Фильтр незавершённых карт по Tier",
      loadMore: "Загрузка дополнительных карт...",
      loadFailed: "Не удалось загрузить незавершённые карты.",
      loadFailedBody: "Обновите страницу и попробуйте снова.",
      emptyNub:
        "Для этого игрока в выбранной области не найдено незавершённых карт.",
      emptyPro:
        "Для этого игрока в выбранной области не найдено незавершённых PRO-карт.",
      typeNub: "NUB",
      typePro: "PRO",
    },
    ban: {
      recordPinned: "Рекорд закреплён",
      pinFailed: "Не удалось закрепить рекорд",
      recordUnpinned: "Рекорд откреплён",
      unpinFailed: "Не удалось открепить рекорд",
      statusUpdated: "Статус бана обновлён",
      noLongerBanned: "Ваш профиль больше не помечен как активно забаненный.",
      statusChecked: "Статус бана проверен",
      activeRemain_one: "Остался {{count}} активный бан.",
      activeRemain_other: "Осталось {{count}} активных банов.",
      statusCheckFailed: "Не удалось проверить статус бана",
      warningTitle: "Этот игрок забанен",
      checking: "Проверка...",
      checkUnbanStatus: "Проверить статус разбана",
      permanent: "Постоянный",
      temporary: "Временный",
      noNotes: "Заметки отсутствуют.",
    },
    stats: {
      title: "Самый играемый сервер",
      mapsTitle: "Самые играемые карты",
      subtitle:
        "Доля времени на рекордах по группе серверов или отдельному серверу.",
      allTime: "За всё время",
      last365Days: "Недавнее",
      play: "Воспроизвести",
      pause: "Пауза",
      totalPlaytime: "Всего {{total}}",
      totalRecords: "Всего {{total}} записей",
      recordsMetric: "Записи",
      hoursMetric: "Часы",
      empty: "Для этого представления не найдено времени на рекордах.",
      mapsEmpty: "Для этого представления не найдены данные по картам.",
      loadFailed: "Не удалось загрузить статистику.",
      ariaLabel: "Диаграмма самых играемых серверов для {{view}}",
      mapsAriaLabel: "Диаграмма самых играемых карт для {{view}}",
      tooltipHours: "Время игры: {{hours}} {{percent}}%",
      tooltipRecords_one: "{{count}} запись",
      tooltipRecords_few: "{{count}} записи",
      tooltipRecords_many: "{{count}} записей",
      tooltipRecords_other: "{{count}} записи",
      tooltipServers_one: "{{count}} сервер",
      tooltipServers_few: "{{count}} сервера",
      tooltipServers_many: "{{count}} серверов",
      tooltipServers_other: "{{count}} сервера",
    },
    placeholder: {
      badge: "Скоро",
      recordsHeading: "Рекорды игрока {{name}}",
      statsHeading: "Статистика игрока {{name}}",
      description:
        "Маршрут уже доступен публично, но в этой первой версии содержимое намеренно минимально. Общая оболочка профиля и структура URL уже готовы для будущих вкладок с данными от backend.",
    },
  },
  reviews: {
    title: "Отзывы",
    moderationWarning:
      "Вы можете свободно высказать недовольство картой и указать, что делает её неинтересной, но говорите о карте, а не об авторе. Оскорбления или личные нападки на автора и его семью запрещены.",
    withCommentsOnly: "Только с комментариями",
    allLanguages: "Все языки",
    languages: {
      all: "Все языки",
      en: "English",
      zh: "中文",
      ru: "Русский",
    },
    emptyFiltered: "Нет отзывов, подходящих под текущие фильтры.",
    empty: "Отзывы не найдены.",
    loadFailed: "Не удалось загрузить отзывы",
    noRatingAria: "Оценка не указана, 0 из 5 звёзд",
    ratingAria: "{{count}} из 5 звёзд",
    noReviewsAria: "Нет отзывов, 0 из 5 звёзд",
    reviewsAria: "{{rating}} из 5 звёзд по {{count}} отзывам",
  },
  leaderboards: {
    tabs: {
      players: "Игроки",
      community: "Сообщество",
      pow: "POW",
      jumpstats: "Jumpstats",
      servers: "Серверы",
      maps: "Карты",
      countries: "Страны",
    },
    jumpstats: {
      filters: "Фильтры",
      jumpType: "Тип прыжка",
      minRatingAria: "Минимальный рейтинг",
      minRatingValue: "Rating {{value}}+",
      loadFailedTitle: "Не удалось загрузить таблицу jumpstats",
      loadFailedDescription: "Не удалось загрузить таблицу лидеров jumpstats.",
      empty: "Записи для таблицы лидеров jumpstats не найдены.",
      dialog: {
        title: "Детали jumpstat",
        description:
          "Маршрут, разбивка по стрейфам и визуализация движения из реплея.",
        copyDetails: "Скопировать детали",
        copyLabel: "детали jumpstat",
        units: "юн.",
        breakdownTitle: "Разбивка по стрейфам",
        deviation: "Отклонение",
        detailLoadFailedTitle: "Не удалось загрузить детали jumpstat",
        detailLoadFailedBody: "Перезагрузите страницу и попробуйте снова.",
        visualizationLoadFailedTitle: "Не удалось загрузить маршрут прыжка",
        visualizationLoadFailedBody:
          "Не удалось построить визуализацию реплея для этого jumpstat.",
        visualization: {
          title: "Визуализация маршрута",
          deviationAngle: "Угол отклонения",
          legend: "Цвета луча",
          neutral: "Нейтрально",
          gain: "Прирост",
          loss: "Потеря",
          duck: "Присед",
          aKey: "A",
          dKey: "D",
          mouse: "Мышь",
        },
        summary: {
          strafes: "Стрейфы",
          sync: "Синк",
          pre: "Пре",
          max: "Макс",
          width: "Ширина",
          height: "Высота",
          airtime: "Аиртайм",
          offset: "Смещение",
          w: "W",
          ol: "OL",
          da: "DA",
          crouched: "Присед",
        },
        table: {
          sync: "Синк",
          gain: "Прирост",
          loss: "Потеря",
          airtime: "Аиртайм",
          width: "Ширина",
          ol: "OL",
          da: "DA",
        },
      },
      columns: {
        distance: "Distance",
        block: "Block",
        strafes: "Strafes",
        sync: "Sync",
        pre: "Pre",
        max: "Max",
        jumpedAt: "Jumped At",
      },
      types: {
        LJ: "Long Jump",
        BH: "Bhop",
        MBH: "Multi Bunnyhop",
        WJ: "Weird Jump",
        LAJ: "Ladder Jump",
        LAH: "Ladderhop",
        JB: "Jumpbug",
        LBH: "Lowpre Bunnyhop",
        LWJ: "Lowpre Weird Jump",
        FL: "Fall",
        UNK: "Unknown",
        INV: "Invalid",
      },
    },
    players: {
      skills: {
        select: "Выбрать навык, сейчас {{skill}}",
        rawRating: "Исходный рейтинг: {{rating}}",
      },
      friends: {
        label: "Друзья",
        loginRequiredTitle: "Требуется вход",
        loginRequiredDescription:
          "Сначала войдите в аккаунт, чтобы видеть таблицу лидеров друзей.",
      },
      findMe: {
        loginRequiredTitle: "Требуется вход",
        loginRequiredDescription:
          "Сначала войдите в аккаунт, чтобы найти себя в таблице лидеров.",
      },
    },
    community: {
      title: "Рейтинги сообщества",
      subtitle: "Общая активность вокруг профилей на GOKZ.TOP.",
      metricAria: "Выбрать рейтинг сообщества",
      entries: "записей",
      empty: "Записи рейтинга сообщества не найдены.",
      loadFailedTitle: "Не удалось загрузить рейтинги сообщества",
      metrics: {
        platformFollowers: "Подписчики платформы",
        viewsCount: "Просмотры",
        uniqueVisitors: "Уникальные посетители",
        likes: "Лайки",
        uniqueLikers: "Уникальные лайкнувшие",
      },
    },
    countries: {
      title: "Рейтинг стран и регионов",
      country: "Страна/регион",
      countryTooltip: "Страна или регион, указанные для игроков в рейтинге.",
      rankedPlayers: "Игроки в рейтинге",
      activePlayers: "Активные игроки",
      activePlayersTooltip: "Игроки, игравшие за последние 30 дней.",
      topPlayers: "Лучшие игроки",
      viewTopPlayers: "Показать 10 лучших игроков",
      top10PercentileRating: "Рейтинг топ-10%",
      top10PercentileRatingTooltip:
        "Минимальный рейтинг среди лучших 10% игроков этой страны.",
      top10AverageRating: "Средний рейтинг топ-10",
      empty: "Записи рейтинга стран не найдены.",
      loadFailedTitle: "Не удалось загрузить рейтинг стран",
    },
    mapColumns: {
      playtime: "Время в игре",
      averagePlaytime: "Среднее время",
      proRatio: "Доля PRO",
      finishes: "Финиши",
      firstMedian: "1-й медиана",
    },
  },
  scope: {
    select: "Выбрать область рекордов",
  },
  pagination: {
    total: "Всего",
    entries: "записей",
    rowsPerPage: "Строк на странице",
    currentPage: "Текущая страница: {{page}} из {{pageCount}}",
    currentPageShort: "Текущая страница",
    first: "Перейти на первую страницу",
    previous: "Перейти на предыдущую страницу",
    next: "Перейти на следующую страницу",
    last: "Перейти на последнюю страницу",
    page: "Страница",
    of: "из",
  },
  dateTime: {
    presets: {
      iso: {
        label: "ISO-подобный",
        description:
          "Формат с годом в начале для максимально ясной сортировки.",
      },
      us: {
        label: "США",
        description: "Компактный формат месяц/день.",
      },
      euro: {
        label: "Европа",
        description: "Компактный формат день/месяц.",
      },
      long: {
        label: "Полный",
        description: "Более читаемый формат с названием месяца.",
      },
    },
    hourCycle: {
      h24: {
        label: "24-часовой",
        description: "Показывает время как 14:05.",
      },
      h12: {
        label: "12-часовой",
        description: "Показывает время как 2:05 PM.",
      },
    },
    relative: {
      year_one: "{{count}} год назад",
      year_other: "{{count}} лет назад",
      month_one: "{{count}} месяц назад",
      month_other: "{{count}} месяцев назад",
      week_one: "{{count}} неделю назад",
      week_other: "{{count}} недель назад",
      day_one: "{{count}} день назад",
      day_other: "{{count}} дней назад",
      hour_one: "{{count}} час назад",
      hour_other: "{{count}} часов назад",
      minute_one: "{{count}} мин назад",
      minute_other: "{{count}} мин назад",
      second_one: "{{count}} секунду назад",
      second_other: "{{count}} секунд назад",
      inYear_one: "через {{count}} год",
      inYear_other: "через {{count}} лет",
      inMonth_one: "через {{count}} месяц",
      inMonth_other: "через {{count}} месяцев",
      inWeek_one: "через {{count}} неделю",
      inWeek_other: "через {{count}} недель",
      inDay_one: "через {{count}} день",
      inDay_other: "через {{count}} дней",
      inHour_one: "через {{count}} час",
      inHour_other: "через {{count}} часов",
      inMinute_one: "через {{count}} мин",
      inMinute_other: "через {{count}} мин",
      inSecond_one: "через {{count}} секунду",
      inSecond_other: "через {{count}} секунд",
      todayAt: "Сегодня в {{time}}",
      yesterdayAt: "Вчера в {{time}}",
      oneHourMinutesAgo: "1 час {{minutes}} мин назад",
    },
  },
  maps: {
    title: "Карты",
    loaded: "Загружено карт: {{count}}",
    visible: "Показано карт: {{count}}",
    openWorkshop: "Открыть Workshop",
    authors: "Авторы",
    workshopId: "Workshop ID",
    created: "Создано",
    updated: "Обновлено",
    zoomImage: "Увеличить изображение карты {{mapName}}",
    imageAlt: "Превью карты {{mapName}}",
    imageAltEnlarged: "Увеличенное превью карты {{mapName}}",
    unknownAuthor: "Автор неизвестен",
    requestFailed:
      "Не удалось загрузить каталог карт. Обновите список и попробуйте снова.",
    noResultsTitle: "По этому запросу карты не найдены",
    noResultsBody:
      "Попробуйте другое имя или ослабьте фильтр, чтобы увидеть больше результатов.",
    previous: "Назад",
    next: "Вперёд",
    searchPlaceholder: "Поиск карт...",
    searchAria: "Искать карты по названию",
    validated: "Проверенные",
    invalid: "Недействительные",
    validatedStatusAria:
      "Показаны проверенные карты. Нажмите, чтобы показать недействительные карты.",
    invalidStatusAria:
      "Показаны недействительные карты. Нажмите, чтобы показать проверенные карты.",
    showFilters: "Фильтры",
    hideFilters: "Скрыть фильтры",
    filterTitle: "Фильтры",
    clearFilters: "Очистить фильтры",
    filterMin: "Мин.",
    filterMax: "Макс.",
    invalidWrTime: "Введите время WR в секундах или в формате М:СС.ммм.",
    invalidDate: "Введите корректную дату.",
    invalidNumber: "Введите неотрицательное число.",
    invalidRange: "Минимум не может быть больше максимума.",
    filterFields: {
      wrTime: "Время WR",
      tier: "Tier",
      createdAt: "Дата создания",
      updatedAt: "Дата обновления",
      overallRating: "Общая оценка",
      reviewCount: "Число отзывов",
      commentsCount: "Число комментариев",
      playtime: "Время игры",
      avgPlaytime: "Ср. время игры",
      nub: "NUB",
      pro: "PRO",
      proRatio: "Доля PRO",
      finishes: "Прохождения",
      firstMed: "Медиана 1-го",
      metrics: "Метрики",
      nubTooltip: "Число уникальных игроков, завершивших карту в NUB.",
      proTooltip: "Число уникальных игроков, завершивших карту в PRO.",
    },
    filterTier: "Фильтр карт по Tier",
    minimumTierAria: "Минимальный Tier карты",
    maximumTierAria: "Максимальный Tier карты",
    withBonus: "С бонусами",
    withBonusAria: "Показать только карты с бонусами",
    downloadDialog: {
      button: "Скачать",
      title: "Скачать карты",
      description: "Скачать или обновить карты одной командой.",
      linuxLabel: "Linux / MacOS",
      windowsLabel: "Windows",
      copyCommand: "Скопировать команду {{label}}",
      commandCopied: "Команда скопирована",
      runFrom: "Запустите эту команду из вашей папки csgo.",
    },
    autoUpdateDialog: {
      button: "Автообновление",
      title: "Автоматическое обновление карт",
      description:
        "Поддерживайте проверенные карты в актуальном состоянии с помощью ежечасной задачи cron.",
      step1Title: "Проверьте программу обновления",
      step1Description:
        "Один раз запустите обновление из папки csgo/, а не csgo/maps/, и убедитесь, что оно завершилось успешно.",
      step2Title: "Откройте crontab пользователя сервера",
      step2Description:
        "Выполните команду от имени пользователя Linux, которому принадлежит игровой сервер.",
      step3Title: "Добавьте ежечасную задачу",
      step3Description:
        "Вставьте команду с новой строки, затем сохраните и закройте редактор.",
      manualCommandLabel: "Ручное обновление",
      crontabCommandLabel: "Открыть crontab",
      cronCommandLabel: "Ежечасная задача cron",
      customCronCommandLabel: "Ежечасная задача cron с нестандартным путём",
      logCommandLabel: "Последние журналы обновления",
      copyCommand: "Скопировать: {{label}}",
      commandCopied: "Команда скопирована",
      behaviorTitle: "Как работает задача",
      runsHourly:
        "Проверяет карты в начале каждого часа, поэтому новая карта может появиться с задержкой до часа.",
      serverOwner: "Работает с правами владельца игрового сервера.",
      autoDetect: "Автоматически находит стандартный путь LinuxGSM",
      downloadBehavior:
        "Скачивает только отсутствующие или изменённые проверенные карты.",
      lockBehavior:
        "Блокировка предотвращает параллельные обновления, а вывод записывается в $HOME/map-update.log.",
      customPathTitle: "Сервер установлен в нестандартном месте?",
      customPathDescription:
        "Скопируйте этот вариант и замените /path/to/serverfiles/csgo абсолютным путём к папке csgo/:",
    },
    bonusCount_one: "{{count}} бонус",
    bonusCount_few: "{{count}} бонуса",
    bonusCount_many: "{{count}} бонусов",
    bonusCount_other: "{{count}} бонуса",
    sortMaps: "Сортировка карт",
    sortMapsByReview: "Сортировка карт по отзывам",
    sortMapsBySkill: "Сортировка карт по навыку",
    pagesAria: "Страницы карт",
    goToPage: "Перейти на страницу {{page}}",
    sortOptions: {
      name: "Название",
      tier: "Tier",
      created: "Создано",
      updated: "Обновлено",
      wr: "WR",
      review: "Отзыв",
      bonus: "Бонус",
      skill: "Навык",
      bestRated: "Лучшие по рейтингу",
      overall: "Общее",
      gameplay: "Геймплей",
      visuals: "Визуал",
      reviewCount: "Число отзывов",
      commentsCount: "Число комментариев",
      playtime: "Время игры",
      avgPlaytime: "Ср. время игры",
      nub: "NUB",
      pro: "PRO",
      proRatio: "Доля PRO",
      finishes: "Прохождения",
      firstMed: "Медиана 1-го",
      metrics: "Метрики",
      adjustedRating: "Рейтинг:",
      adjustedRatingTooltip:
        "Сочетает рейтинг и число отзывов для большей точности.",
      nubTooltip: "Число уникальных игроков, завершивших карту в NUB.",
      proTooltip: "Число уникальных игроков, завершивших карту в PRO.",
    },
    tabs: {
      top: "Топ карты",
      stats: "Статистика",
      wrHistory: "История WR",
      reviews: "Отзывы",
    },
    wrHistory: {
      title: "История мирового рекорда",
      loadFailedBody:
        "Не удалось загрузить историю WR. Попробуйте обновить страницу.",
      empty: "Для этого представления пока нет событий мирового рекорда.",
      xAxis: {
        time: "Дата",
        record: "Каждая запись",
      },
    },
    stats: {
      loadFailedTitle: "Не удалось загрузить статистику карты",
      loadFailedBody: "Попробуйте обновить страницу.",
      nubTitle: "Распределение WR Gap NUB",
      proTitle: "Распределение WR Gap PRO",
      medianLabel: "Медиана: {{median}} ({{time}})",
      noWr: "Нет WR",
      empty: "Для этого среза пока нет доступных PB gap.",
      medianMarker: "Медиана",
      youMarker: "Вы",
      tooltipCount: "{{count}} PB",
      tooltipCountWithPercent: "{{count}} PB ({{percent}})",
      tooltipTimeRange: "Диапазон времени: {{start}} - {{end}}",
    },
    filters: {
      country: "страна",
      region: "регион",
      mainStage: "Основной",
      bonusStage: "Бонус {{stage}}",
    },
    addReview: "Добавить отзыв",
    nubFinishers: "Финишировавших NUB",
    proFinishers: "Финишировавших PRO",
    topPercentPrefix: "Топ",
    myRank: "Мой ранг",
    findMe: "Найти меня",
    wrGapInfoButtonAria: "Объяснить WR Gap",
    wrGapInfoTitle: "Что такое WR Gap?",
    wrGapInfoDescription:
      "WR Gap показывает, насколько результат отстаёт от мирового рекорда, на логарифмической шкале.",
    wrGapInfoFormulaLabel: "Формула",
    wrGapInfoCloser:
      "Чем число более отрицательное, тем ближе результат к WR. Чем ближе к 0, тем дальше он от WR.",
    wrGapInfoExampleLabel: "Пример",
    wrGapInfoExample:
      "Если WR равен 100.00 секундам, а результат равен 125.00 секундам, лишние 25% дают log2(0.25) = -2.00.",
    wrGapInfoWrRow:
      "В строке WR показывается '-', потому что для самого WR формула не определена.",
    findMeNotRankedTitle: "У вас нет ранкнутого финиша здесь",
    findMeNotRankedDescription:
      "Попробуйте другую область, фильтр или другой режим NUB / PRO.",
    emptyTopPro:
      "Для этой карты нет Stage {{stage}} записей PRO в выбранной области.",
    emptyTop: "Для этой карты нет Stage {{stage}} записей в выбранной области.",
  },
  compare: {
    title: "Сравнение игроков",
    description:
      "Сравните рейтинги, прогресс карт и PB основных этапов в выбранном скоупе.",
    player1: "Игрок 1",
    player2: "Игрок 2",
    player1Search: "Найти игрока 1",
    player2Search: "Найти игрока 2",
    clearPlayer1: "Очистить игрока 1",
    clearPlayer2: "Очистить игрока 2",
    swapPlayers: "Поменять игроков",
    duplicatePlayers: "Выберите двух разных игроков.",
    selectBoth: "Выберите двух игроков для сравнения.",
    loading: "Загрузка сравнения...",
    loadFailed: "Не удалось загрузить сравнение.",
    globalRank: "Глобальное место",
    ratingEasy: "Rating E",
    ratingHard: "Rating H",
    nubWrs: "NUB WR",
    proWrs: "PRO WR",
    records900: "PB 900+",
    records800: "PB 800+",
    progression: "Прогресс карт",
    progressionDescription: "Процент завершённых основных карт по тиру.",
    progressionAria:
      "Сравнение завершения тиров между {{player1}} и {{player2}}",
    tier: "Tier",
    runs: "Забеги",
    onlyBoth: "Только карты, завершённые обоими",
    noRuns: "В этом представлении нет PB основного этапа.",
    timeDelta: "Разница времени",
    pointsDelta: "Разница очков",
  },
  live: {
    title: "Live",
    online: "Онлайн",
    toggleHint:
      "Нажмите, чтобы переключаться между онлайн и офлайн трансляциями",
  },
  reactions: {
    loading: "Загрузка реакций",
    loadError: "Не удалось загрузить реакции.",
    empty: "Пока никто не отреагировал.",
    showAllReactors: "Показать всех: {{count}}",
    showLess: "Свернуть",
    toggle: "Переключить реакцию {{emoji}}",
    reactorsHint:
      "Наведите ненадолго или нажмите правой кнопкой, чтобы увидеть реакцию {{emoji}}",
    reactors: "Реакции",
    reactorCount: "Отреагировал {{count}} игрок",
    reactorCount_other: "Отреагировали {{count}} игроков",
    updateError: "Не удалось сохранить вашу реакцию.",
    add: "Добавить реакцию",
    react: "Реакция",
    choose: "Выберите реакцию",
    signInHint: "Выберите эмодзи, войдите через Steam и вернитесь сюда.",
    catalogError: "Не удалось загрузить список эмодзи.",
  },
  bans: {
    title: "Баны",
    searchAria: "Поиск игроков",
    searchPlaceholder: "Искать игрока ...",
    clearFilter: "Сбросить фильтр игрока",
    searchingPlayers: "Поиск игроков...",
    searchUnavailable: "Сейчас поиск игроков недоступен.",
    noPlayers: "Игроки не найдены.",
  },
} as const
