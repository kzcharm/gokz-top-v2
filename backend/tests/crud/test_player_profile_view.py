...
@pytest.mark.asyncio
async def test_create_player_profile_view_is_idempotent_per_utc_day(
    db: AsyncSession,
) -> None:
    ...
    assert (
        await crud.count_player_profile_views(
            session=db,
            target_steamid64=target.steamid64,
        )
        == 1
...
@pytest.mark.asyncio
async def test_create_player_profile_view_counts_unique_viewer_days(
    db: AsyncSession,
) -> None:
    viewer = await crud.get_or_create_user_from_steam(
        session=db,
...
@pytest.mark.asyncio
async def test_create_player_profile_view_ignores_self_views(db: AsyncSession) -> None:
    viewer = await crud.get_or_create_user_from_steam(
        session=db,
        steamid64=random_steamid64(),
    )
...
def test_get_utc_today_uses_utc_boundaries() -> None:
    assert crud.get_utc_today(now=datetime(2026, 4, 4, 23, 59, tzinfo=UTC)) == datetime(
        2026, 4, 4, 23, 59, tzinfo=UTC
    ).date()
    assert crud.get_utc_today(now=datetime(2026, 4, 5, 0, 0, tzinfo=UTC)) == datetime(
