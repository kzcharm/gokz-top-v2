...
async def test_country_leaderboard_aggregates_activity_and_top_players(
    ...
    assert [row["country"] for row in payload["data"]] == ["US", "DE"]
    assert payload["data"][0]["rank"] == 1
    assert payload["data"][0]["ranked_players"] == 11
    assert payload["data"][0]["active_players"] == 10
    assert payload["data"][0]["top10_percentile_rating"] == pytest.approx(
        scale_public_rating(1900)
    )
    assert "percentile10_rating" not in payload["data"][0]
    assert payload["data"][0]["top_players"][0]["display_name"] == "US Player 10"
